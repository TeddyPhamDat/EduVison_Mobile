...
class _ContentGenerationScreenState extends State<ContentGenerationScreen> {
  ...
  String _selectedMode = 'video'; // Default to video since it gives both video and slides
  // Available options
  List<String> _availableSubjects = [];
  List<String> _availableChapters = [];
  
  final List<String> _availableGrades = ['12']; // Only grade 12 is available based on API
  ...
  Future<void> _loadChapters() async {
    ...
    try {
      ...
      setState(() {
        ...
        if (_selectedMode.toLowerCase() == 'video' && result.containsKey('videoUrl')) {
          _generationResult = result['videoUrl'];
          
          // Save to history
          _contentStorageService.saveContent(
            subject: _selectedSubject!,            chapter: _selectedChapter!,
            grade: _selectedGrade,
            contentType: 'video',
        ...
        if (result.containsKey('slideUrl') && result.containsKey('videoUrl')) {
          ...
          final String contentType = _selectedMode.toLowerCase() == 'video' ? 'slides' : 'video';
          final String url = _selectedMode.toLowerCase() == 'video' ? result['slideUrl'] : result['videoUrl'];
              ...
              Navigator.push(
                ...
                CupertinoPageRoute(
                  builder: (context) => ContentViewerScreen(
                    url: url,
                    title: '$contentType ${ _selectedSubject ?? ""} - ${_selectedChapter ?? ""}',
                    contentType: contentType,
                  ),
                ),
              );
                      ...
                      children: [
                        ...
                        _buildDropdown(
                          ...
                          onChanged: (String? value) {
                            if (value != null) {
                              setState(() {
                                _selectedTemplate = int.parse(value.replaceAll('Mẫu ', ''));
                              });
                            }
                          },
